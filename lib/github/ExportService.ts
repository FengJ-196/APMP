...
export class ExportService {
  ...
  static async exportWBSItemToGitHub(
    ...
    try {
      // 7. Post issue creation request to GitHub
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 && integration.encryptedRefreshToken) {
          try {
            console.log('GitHub access token expired. Attempting token refresh...');
            ...
            console.error('GitHub automatic token refresh/retry failed:', refreshErr);
            throw new Error(`GitHub token expired and refresh failed: ${refreshErr.message}`);
          }
        }
      ...
      // 8. Update or Create ExternalSync document to 'synced'
      ...
      // 9. On failure, update or create ExternalSync document with status 'failed'
      try {
